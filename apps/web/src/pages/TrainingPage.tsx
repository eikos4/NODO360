...
const CATEGORY_COLORS: Record<string, string> = {
  LICENCIA: 'bg-sky-50 dark:bg-sky-500/20 text-sky-600 dark:text-sky-400 border-sky-200 dark:border-sky-500/30',
  EPP: 'bg-orange-50 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 border-orange-200 dark:border-orange-500/30',
  MEDICO: 'bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/30',
  CURSO: 'bg-violet-50 dark:bg-violet-500/20 text-violet-600 dark:text-violet-400 border-violet-200 dark:border-violet-500/30',
  HABILITACION: 'bg-amber-50 dark:bg-amber-500/20 text-amber-600 dark:text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-500/30',
  OTRO: 'bg-slate-500/20 text-slate-600 dark:text-slate-400 border-slate-500/30',
};

const STATUS_META: Record<CertStatus, { label: string; badge: string; dot: string }> = {
  VIGENTE: { label: 'Vigente', badge: 'bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/30', dot: 'bg-emerald-400' },
  POR_VENCER: { label: 'Por vencer', badge: 'bg-amber-50 dark:bg-amber-500/20 text-amber-600 dark:text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-500/30', dot: 'bg-amber-400' },
  VENCIDO: { label: 'Vencido', badge: 'bg-red-50 dark:bg-red-500/20 text-red-600 dark:text-red-600 dark:text-red-400 border-red-200 dark:border-red-500/30', dot: 'bg-red-400' },
  SIN_VENCIMIENTO: { label: 'Sin vencimiento', badge: 'bg-slate-500/20 text-slate-600 dark:text-slate-400 border-slate-500/30', dot: 'bg-slate-400' },
};

const inputCls =
  'w-full bg-white dark:bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-600 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500/50 transition-all';
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 max-w-7xl mx-auto">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-semibold text-red-600 dark:text-red-400 uppercase tracking-widest mb-1">Personal</p>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <GraduationCap className="w-7 h-7 text-red-600 dark:text-red-400" />
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="bg-white dark:bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-slate-200 min-w-[200px]"
          >
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-t-xl text-sm font-semibold transition-colors ${
                tab === t.id
                  ? 'bg-red-100 dark:bg-red-600/20 text-red-600 dark:text-red-600 dark:text-red-300 border border-red-600/30 border-b-transparent'
      ...
      {tab === 'resumen' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            {[
              { label: 'Total', value: summary?.total ?? 0, icon: GraduationCap, color: 'text-slate-600 dark:text-slate-300' },
              { label: 'Vigentes', value: summary?.vigente ?? 0, icon: CheckCircle2, color: 'text-emerald-600 dark:text-emerald-400' },
              { label: 'Por vencer', value: summary?.porVencer ?? 0, icon: Clock, color: 'text-amber-600 dark:text-amber-400' },
              { label: 'Vencidos', value: summary?.vencido ?? 0, icon: AlertTriangle, color: 'text-red-600 dark:text-red-400' },
              { label: 'Sin fecha', value: summary?.sinVencimiento ?? 0, icon: FileText, color: 'text-slate-600 dark:text-slate-400' },
            ].map(k => {
              const Icon = k.icon;
              return (
                <div key={k.label} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none rounded-2xl p-4">
                  <Icon className={`w-5 h-5 ${k.color} mb-2`} />
                  <p className="text-2xl font-bold text-slate-900 dark:text-white">{k.value}</p>
                  <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-500 uppercase tracking-wide">{k.label}</p>
                </div>
              );
            })}
          </div>
          ...
          {(expiring?.expired?.length > 0 || expiring?.soon?.length > 0) && (
            <div className="grid gap-4 lg:grid-cols-2">
              {expiring?.expired?.length > 0 && (
                <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/40 rounded-2xl p-4">
                  <h3 className="text-sm font-bold text-red-600 dark:text-red-300 flex items-center gap-2 mb-3">
                    <AlertTriangle className="w-4 h-4" /> Vencidos ({expiring.expired.length})
                  </h3>
                  <ul className="space-y-2 max-h-64 overflow-y-auto">
                    {expiring.expired.map((c: any) => (
                      <li key={c.id} className="text-sm bg-white dark:bg-slate-900/80 shadow-sm dark:shadow-none rounded-xl px-3 py-2 border border-slate-200 dark:border-slate-800">
              ...
              {expiring?.soon?.length > 0 && (
                <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/30 rounded-2xl p-4">
                  <h3 className="text-sm font-bold text-amber-600 dark:text-amber-300 flex items-center gap-2 mb-3">
          ...
          {summary?.byCategory && Object.keys(summary.byCategory).length > 0 && (
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none rounded-2xl p-4">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Por categoría</h3>
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap gap-3">
            ...
            <select
              ...
              className="bg-white dark:bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-900 dark:text-slate-200"
            >
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none rounded-2xl text-slate-500 dark:text-slate-500">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {filteredCerts.map((c: any) => {
                ...
                return (
                  <div key={c.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none rounded-2xl p-4 hover:border-slate-200 dark:border-slate-700 transition-colors">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      ...
                      {c.issuer && <span className="text-[10px] text-slate-500 dark:text-slate-500">{c.issuer}</span>}
                    </div>
                    <div className="flex flex-wrap gap-3 text-xs text-slate-500 dark:text-slate-500 mb-3">
                    ...
                    {c.daysUntilExpiry != null && c.status !== 'SIN_VENCIMIENTO' && (
                      <p className={`text-xs mb-3 ${c.status === 'VENCIDO' ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'}`}>
                        {c.status === 'VENCIDO' ? `Venció hace ${Math.abs(c.daysUntilExpiry)} días` : `${c.daysUntilExpiry} días restantes`}
                      </p>
                    )}
                    {canEdit && (
                      <div className="flex gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                        <button onClick={() => openEdit(c)} className="text-xs text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white flex items-center gap-1">
                          <Pencil className="w-3.5 h-3.5" /> Editar
                        </button>
                        <button onClick={() => remove.mutate(c.id)} className="text-xs text-red-600 dark:text-red-400 hover:text-red-600 dark:text-red-300 flex items-center gap-1">
      ...
      {tab === 'bomberos' && (
        <div className="space-y-4">
          {!companyFilter ? (
            <div className="text-center py-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none rounded-2xl text-slate-500 dark:text-slate-500">
              <Building2 className="w-10 h-10 mx-auto mb-2 opacity-40" />
              <p>Selecciona una compañía para ver el roster</p>
            </div>
          ) : roster.length === 0 ? (
            <div className="text-center py-12 text-slate-500 dark:text-slate-500">Sin personal activo en esta compañía</div>
          ) : (
            <div className="space-y-2">
              {roster.map((u: any) => {
                ...
                return (
                  <div key={u.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none rounded-2xl overflow-hidden">
                    <button
                      type="button"
                    ...
                    >
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm ${hasAlert ? 'bg-red-100 dark:bg-red-600/20 text-red-600 dark:text-red-600 dark:text-red-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
                        {u.firstName[0]}{u.lastName[0]}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-900 dark:text-white">{u.firstName} {u.lastName}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-500">{u.rut} · {u.certifications.length} certificación(es)</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {u.stats.vencido > 0 && (
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/30">
                            {u.stats.vencido} venc.
                          </span>
                        )}
                        {u.stats.porVencer > 0 && (
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-500/30">
                    ...
                    {open && (
                      <div className="px-4 pb-4 space-y-2 border-t border-slate-200 dark:border-slate-800 pt-3">
                        {u.certifications.length === 0 ? (
                          <p className="text-sm text-slate-500 dark:text-slate-500">Sin certificaciones</p>
                        ) : (
                          u.certifications.map((c: any) => {
                            const st = STATUS_META[c.status as CertStatus] ?? STATUS_META.VIGENTE;
                            return (
                              <div key={c.id} className="flex items-center justify-between gap-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-transparent rounded-xl px-3 py-2 text-sm">
                                <div>
                                  <p className="text-slate-900 dark:text-white font-medium">{c.name}</p>
                                  <p className="text-xs text-slate-500 dark:text-slate-500">Vence: {fmt(c.expiresAt)}</p>
                                </div>
                        ...
                        {canEdit && (
                          <button
                            ...
                            className="text-xs text-red-600 dark:text-red-400 hover:text-red-600 dark:text-red-300 font-semibold mt-1"
                          >
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full shadow-2xl max-w-lg max-h-[90vh] overflow-y-auto p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">{editing ? 'Editar certificación' : 'Nueva certificación'}</h2>
              <button onClick={resetForm} className="text-slate-500 dark:text-slate-500 hover:text-slate-900 dark:text-white"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-slate-500 dark:text-slate-500 mb-1 block">Fecha emisión</label>
                <input type="date" className={inputCls} value={form.issuedAt} onChange={e => setForm({ ...form, issuedAt: e.target.value })} />
              </div>
              <div>
                <label className="text-xs text-slate-500 dark:text-slate-500 mb-1 block">Fecha vencimiento</label>
