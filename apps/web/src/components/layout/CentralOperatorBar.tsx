...
import { Flame, LogOut, Moon, Siren, Map, ShieldAlert, Radio, Sun, Zap } from 'lucide-react';
import { useAuthStore } from '../../store/authStore';
import { useThemeStore } from '../../store/themeStore';
import { cn } from '../../lib/utils';
import CentralCompanias360Menu from './CentralCompanias360Menu';
...
export default function CentralOperatorBar() {
  ...
  return (
    <header className={cn(
      ...
      <div className="h-12 sm:h-14 flex items-center justify-between gap-2 px-3 sm:px-4">
        ...
        <nav className="flex items-center gap-0.5 sm:gap-1 flex-1 justify-center max-w-xl lg:max-w-2xl overflow-x-auto scrollbar-thin">
          {TABS.map(({ to, label, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                to={to}
                className={cn(
                  'flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-[10px] sm:text-xs font-semibold transition-colors shrink-0',
                  active
