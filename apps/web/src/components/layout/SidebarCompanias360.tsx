...
import {
  COMPANIAS360,
  companias360Path,
  filterCompanias360ForRole,
  isCompanias360Path,
} from '../../lib/companias360';
...
export default function SidebarCompanias360({ compact, roles, userRole, companyNumber }: Props) {
  ...
  const list = useMemo(
    () => filterCompanias360ForRole(userRole, companyNumber),
    [userRole, companyNumber],
  );
  const singleCompany = list.length === 1;
  const menuTitle = singleCompany ? 'Mi Compañía' : 'Compañías360';
  ...
  if (singleCompany) {
    ...
    return (
      ...
      >
        ...
        {!compact && (
          <>
            <span className="flex-1 text-left truncate">{menuTitle}</span>
            <span className="w-5 h-5 rounded-md bg-red-700 text-white text-[10px] font-black flex items-center justify-center shrink-0">
              {c.number}
            </span>
  ...
  return (
    <div ref={wrapRef} className="relative mb-0.5">
      <button
        ...
        title={compact ? menuTitle : undefined}
        className={cn(
      ...
      >
        ...
        {!compact && (
          <>
            <span className="flex-1 text-left">{menuTitle}</span>
            <span className={cn('text-[9px] font-bold px-1.5 py-0.5 rounded', active ? 'bg-red-700 text-white' : 'bg-slate-800 text-slate-500')}>
              {list.length}
            </span>
            {open ? (
      ...
      {compact && flyout && (
        <div className="absolute left-full top-0 ml-2 w-52 py-2 px-1.5 bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-[100]">
          <p className="px-3 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">{menuTitle}</p>
          {list.map(childLink)}
        </div>
      )}
    </div>
