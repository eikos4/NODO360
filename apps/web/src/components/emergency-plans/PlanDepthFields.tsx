  ...
  return (
    <div className="space-y-5 border-t border-slate-800 pt-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">Estado del plan</label>
        ...
        {form.version != null && (
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">Versión actual</label>
            <p className="text-sm font-bold text-sky-400 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5">v{form.version}</p>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {checklist.length === 0 ? (
            <p className="text-xs text-slate-600 py-3 text-center border border-dashed border-slate-700 rounded-xl">Sin ítems — agrega pasos verificables</p>
          ) : (
            checklist.map((item, idx) => (
              <div key={item.id} className="flex gap-2 items-start bg-slate-800/50 border border-slate-700 rounded-xl p-2">
                ...
                <input
                  ...
                  className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-100"
                />
                <label className="flex items-center gap-1 text-[10px] text-slate-400 shrink-0 pt-2">
                  <input
                    type="checkbox"
        ...
        <>
          <div>
            ...
            <input
              ...
              className="block w-full text-xs text-slate-400 file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-slate-800 file:text-slate-200"
            />
            <ul className="mt-2 space-y-1">
              {attachments.map(a => (
                <li key={a.id} className="flex items-center justify-between gap-2 text-xs bg-slate-800 rounded-lg px-3 py-2">
          ...
          {versions.length > 0 && (
            <div>
              ...
              <ul className="max-h-36 overflow-y-auto space-y-1">
                {versions.map(v => (
                  <li key={v.id} className="text-xs bg-slate-800/80 border border-slate-700 rounded-lg px-3 py-2 flex justify-between gap-2">
                    <span className="text-slate-200 font-medium">v{v.version} — {v.title}</span>
